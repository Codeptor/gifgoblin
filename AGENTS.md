...
**Naming:** the product/repo is **gifgoblin** (github.com/Codeptor/gifgoblin); the Python package, CLI command, and module names stay **gifharvest** everywhere. Don't rename code-level identifiers.
...
- Gif conversion is **on by default** (`CONVERT_TO_GIF=true`) and needs ffmpeg on PATH — posted gifs are real autoplaying/looping `.gif` files. Conversion scales so the **longest side** fits `GIF_MAX_WIDTH` (fit-in-box, never upscales — not width-only). Falls back to mp4 upload automatically if ffmpeg is missing or the resulting gif exceeds the upload limit; `CONVERT_TO_GIF=false` posts raw mp4s.
